...
- Page generation uses one persistent bounded `ThreadPoolExecutor`; do not reintroduce per-batch executor creation or joins. Completion is processed as available, but returned results are normalized to plan order before persistence. The fixed submission pause remains temporary until provider-aware limits land.
