...
### Planner
- `deepdoc/planner/engine.py` — repo-scan entrypoint and bucket-planning orchestration. `plan_docs()` is now a thin orchestrator: it runs `run_phase2_scans()` exactly once on the full scan, calls `build_planning_units()`, and either plans the whole scan directly via `_plan_local()` (zero/one meaningful unit — identical to pre-Slice-A behavior, no prefixing) or plans each unit against a `make_sub_scan()`-filtered view and merges the results with `merge_unit_plans()`. `_plan_local()` holds the original classify→propose→assign→refine body (Steps 1-3.6+) and does not rerun Phase 2 scans — callers own that.
- `deepdoc/planner/partitioning.py` — `PlanningUnit`, `build_planning_units()` (groups `scan.file_summaries` by `scan.file_services`; unclaimed files go to a deterministic `core` unit; collapses to one unit when there's zero or one meaningful service), `make_sub_scan()` (projects a `RepoScan` to one unit's files without mutating the source scan, including a filtered `TopologyMap`), `split_planning_unit()` (deepest-shared-directory grouping, falling back to stable numbered chunks for a flat unit that can't be split by path)
- `deepdoc/planner/merge.py` — `merge_unit_plans()` deterministically merges per-unit `DocPlan`s: namespaces every non-introduction bucket under `{unit-slug}/`, rewrites `depends_on`/`parent_slug`/nav references through the same slug map, keeps the first unit's introduction as the single global one and demotes every other unit's introduction to a namespaced overview bucket, unions skipped/orphaned files, and validates with `validate_plan_contract()` before returning. `missing_files()` reports any scan file owned by neither a bucket nor the skip/orphan lists. Known limitation: global injectors (Start Here, glossary, database/runtime buckets) still run once per unit rather than once globally post-merge — namespacing keeps this contract-valid (no slug collisions) but multi-service repos will see one Start Here/glossary per service instead of one consolidated global page.
