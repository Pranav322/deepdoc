...
**cloud.deepdoc.tech**. See `AGENTS.md`'s "Hosted-generation harness" section
for the local dev setup this deploys from (`web/hosted/` + `hosted-runner/`).
...
| **`deepdocjobs`** (Storage account) + queue **`deepdoc-jobs`** | `deepdoc-main`/`eastus` | The dispatch queue the Worker enqueues to and the Job consumes | account key in the storage-account conn string; queue-scoped add-only SAS given to the Worker |
...
| Worker `deepdoc-hosted` | Cloudflare account | OAuth, repo picker, **enqueue dispatch**, status/project management, vanity-URL site proxy | `wrangler secret` — `GITHUB_CLIENT_ID`, `GITHUB_SECRET_ID`, `QUEUE_MESSAGES_URL` (queue `/messages` endpoint + add-SAS) |
| D1 `deepdoc-hosted-db` (id `ba724999-e3e3-4eb3-bfed-9d85f49dd01e`) | Cloudflare account | Sessions, projects, quotas, OAuth CSRF state, owner/repo→job lookup | n/a — no external creds, schema in `web/hosted/schema.sql` |
| R2 bucket `deepdoc-hosted-sites` | Cloudflare account | **Durable copy of every finished site** (`{owner}/{repo}/…`) **and job status** (`jobs/{id}/status.json`). The Job uploads both; the Worker serves sites and reads status straight from R2 — no compute to poll. | R2 API token (Object Read & Write, scoped to this bucket) — `R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY` as Job secrets |
...
| `cloud.deepdoc.tech` (Worker custom domain) | Cloudflare zone `deepdoc.tech` | Public entry point | n/a |
| `cloud.deepdoc.tech` (Worker custom domain) | Cloudflare zone `deepdoc.tech` | Public entry point | n/a |
| GitHub OAuth App `DeepDoc Cloud (Production)` | github.com/settings/developers | Login for the hosted flow | Client ID/Secret — `wrangler secret` (`GITHUB_CLIENT_ID`/`GITHUB_SECRET_ID`) |

**Not part of this stack** (pre-existing, left untouched): the root `deepdoc.tech`
domain is served by a separate existing Cloudflare Pages project (the
marketing site) — this deployment only adds the `cloud.` subdomain.
