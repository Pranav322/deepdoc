...
### Added
- **Bounded per-service planning units for large multi-service repos.** Monorepos with several distinct services no longer have to fit one whole-repo required inventory (CLASSIFY/PROPOSE/ASSIGN) into a single model budget. `deepdoc/planner/partitioning.py` groups files by detected service into deterministic `PlanningUnit`s, `bound_planning_unit()` recursively splits any unit that fails an exact-token acceptance gate (real tokenizer count against the real rendered prompt, not a file-count guess) until every part fits, and a unit that still can't be split (a single indivisible oversized file) is marked `coarse` and gets a deterministic placeholder page instead of an LLM call it's guaranteed to overflow. Repos with zero or one meaningful service plan exactly as before — same scan object, no prefixing, no merge.
- **Global bucket injection now runs once after merge, not once per service.** Start Here, glossary, setup, debug, and database/runtime/interface buckets used to get duplicated per planning unit on multi-service repos. `_apply_global_plan_stage()` now runs exactly once against the merged plan and the full repo scan, so a multi-service repo gets one consolidated global set instead of one per service.
- **Deterministic merge across planning units.** `deepdoc/planner/merge.py` namespaces every non-introduction bucket under its unit's slug (so two services proposing the same local slug, like `auth`, never collide), rewrites `depends_on`/`parent_slug`/nav references through the same slug map, keeps one global introduction, and unions skipped/orphaned files — validated by `validate_plan_contract()` before it's used.
- **Scan coverage and unsupported-language warnings in the CLI.** `deepdoc generate`/`update` now warn when a repo contains known-but-unparsed languages (`.java`, `.rs`, `.cs`, ...) and print a documented-vs-orphaned/skipped coverage panel right after planning, built from the existing plan and scan data — no extra scanning, just visibility into what silently didn't make it into the docs.
- **Size/binary/generated-file scan guards.** Oversized, binary, and generated source files are now skipped during scan (`scan.max_source_bytes` config) instead of being read into evidence and diluting the token budget.
- **Conservative capability fallback for unknown LLM models.** An unrecognized model alias (custom Azure/self-hosted deployments LiteLLM has no metadata for) no longer hard-fails at client construction. It now falls back to a loud, conservative default (128k context / 16k output reserve) with a `RuntimeWarning`, and `deepdoc init` bakes the resolved default into `.deepdoc.yaml` so it's explicit and editable. Explicit `context_window_tokens`/`output_reserve_tokens`/`base_model` config still wins and silences the warning.
- **Clear CLI error for empty or unsupported-only repos.** Pointing `generate` at an empty directory, or one with only unsupported languages (e.g. a lone `.java` file), now raises a `click.ClickException` naming the supported languages instead of failing later with an opaque plan-contract error.

### Fixed
- **`fit_prompt_sections()` could still exceed the token budget it was supposed to guarantee.** Per-record token estimates are additive, but the real tokenizer recount after joining records isn't (separator/line overhead grows with record count), so the post-loop recount could report an over-budget total without re-trimming. It now trims accepted records from the end (using already-computed per-record counts, no re-render per drop) until the recount is back under budget.
- **Global bucket slugs could collide across monorepo modules.** Two modules independently proposing the same top-level slug (e.g. both calling it `auth`) previously hard-aborted `validate_plan_contract`. Slugs are now globally deduplicated right after merge and before nav shaping, same as the existing endpoint-family dedup.
- **Route dedup kept both claims when two framework detectors matched the same physical route.** A false-positive match (e.g. Flask-style) and the real framework's route (e.g. Django) could both survive `dedupe_endpoints`. Conflicting claims sharing method+line+file are now arbitrated by `FRAMEWORK_PRIORITIES`, keeping only the highest-priority one.
- **NestJS global route prefixes and Laravel fluent route groups weren't composed into the final path.** `app.setGlobalPrefix(...)` is now composed into every handler path when exactly one unambiguous prefix call exists repo-wide, and Laravel's `Route::prefix('x')->group(fn)` fluent style is now recognized alongside the existing array-style `Route::group(['prefix' => 'x'], fn)`.
- **FastAPI phantom endpoints from unrelated decorator calls.** The `@obj.(get|post|...)(...)` decorator regex could match unrelated calls like `@mock.patch("myapp.services.billing.charge")` in any file that also uses FastAPI, emitting a phantom endpoint whose "path" was actually an import string. Decorator matches whose resolved path doesn't start with `/` are now rejected at detection time. Published (non-phantom) endpoints are also now the authoritative source for planner exclusions, evidence indexing, and OpenAPI ownership — raw detected endpoints remain available for diagnostics only.
- **Multi-unit planning could reuse a stale single-unit plan/ledger after an engine upgrade, and could silently drop root-level source files from per-unit evidence.** The planning-engine fingerprint is now bumped whenever merge/namespacing semantics change, forcing a full replan instead of reusing incompatible cached state; and the per-unit file-tree projection now correctly matches files stored under the scan's root (`"."`) directory instead of dropping them.
- **`max_pages` smaller than the number of planning units crashed with a raw traceback.** `_allocate_page_budgets()` now raises a `click.ClickException` naming the shortfall instead of an unhandled `ValueError`.
- **Phase-2 giant-file and integration-normalization LLM prompts weren't budget-fit.** `cluster_giant_file()` sent one unbounded symbol inventory straight to the LLM, and integration normalization capped candidates at a fixed `[:80]` regardless of the model's real context window. Both now go through `fit_prompt_sections()`, so oversized inputs drop records from the end (same prefix-truncation semantics) and print an omitted-count message instead of silently truncating or overflowing.
