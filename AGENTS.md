...
- `scanner/runtime.py::discover_runtime_surfaces()` is gated twice before any detector runs, and both gates are load-bearing for large repos (DD-001/DD-002). **Trust gate:** low-trust kinds (`test`/`fixture`/`example`/`generated`) are dropped via the shared `source_metadata.classify_source_kind()`/`is_low_trust_source_kind()` — never a second path classifier; the engine passes `scan.source_kind_by_file` and the `None` default self-classifies for direct callers. **Language gate:** each framework detector only sees files whose real language (parser verdict from `parsed_files`, `parser/registry.language_for_extension()` as fallback) can host it — Celery/Django get Python, Laravel PHP, Go `.go`, the JS workers JS/TS/Vue — so Python quoted inside a TypeScript prompt string, or ordinary TS `.delay(`/`.connect(`, cannot invent a Python runtime surface. `_link_runtime_workflows()` then prefilters candidates with one `DISPATCH_MARKER_RE` pass (a proven superset of every task-link pattern) instead of the old files × task-names substring sweep, gates each task on its required literal, and skips the scheduler loop for files owning no endpoint (that loop's only effect was already `endpoint_keys`-guarded). Work counters land in `RuntimeScan.scan_stats` (`input_files`/`eligible_files`/`low_trust_files_skipped`/`link_candidate_files`) and print under a dedicated progress label. Keep the three Phase-2.4 labels distinct — artifacts, runtime surfaces, config/env impacts each get their own line, because one shared label previously hid a 14-minute runtime scan behind "Scanning for setup/deploy/test artifacts...".
