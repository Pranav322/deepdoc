    ...
    if props_match:
        ...
        if props_var not in existing_names:
            symbols.append(Symbol(
                ...
                docstring=f"Component props: {', '.join(prop_names)}" if prop_names else "Component props",
                props=prop_names,
                is_exported=True,
            ))
    ...
    if emits_match:
        ...
        if emits_var not in existing_names:
            symbols.append(Symbol(
                ...
                docstring=f"Emitted events: {', '.join(events)}" if events else "Component emits",
                fields=events,
                is_exported=True,
            ))
...
def _extract_composition_refs(script: str, symbols: list[Symbol],
                               existing_names: set[str]) -> None:
    """Extract ref(), reactive(), computed() declarations."""
    # const name = ref(initialValue)
    for m in re.finditer(r"const\s+(\w+)\s*=\s*(ref|reactive|computed|shallowRef|shallowReactive)\s*[<(]", script):
        name = m.group(1)
        api = m.group(2)
