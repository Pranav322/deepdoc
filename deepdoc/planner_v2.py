...
class RepoScan:
    ...
    runtime_scan: Any = None  # RuntimeScan
...
def run_phase2_scans(scan: RepoScan, cfg: dict[str, Any], llm: LLMClient) -> RepoScan:
    ...
    console.print(
        ...
        f"{len(a.ci_artifacts)} CI, {len(a.test_artifacts)} test, {len(a.ops_artifacts)} ops{db_info}{runtime_info}"
    )
...
def _ensure_database_runtime_and_interface_buckets(
    ...
    plan.buckets = _replace_specialized_buckets(
        ...
        prompt_styles={"database", "database_overview", "database_group", "runtime", "runtime_overview", "graphql"},
    )
...
def _build_database_buckets(scan: RepoScan, cfg: dict[str, Any]) -> list[DocBucket]:
    ...
    db_files = list(
        dict.fromkeys(
            ...
            + [artifact.file_path for artifact in getattr(db_scan, "knex_artifacts", [])]
        )
    )
    if not db_files:
        return []
    ...
    should_split = (
        cfg.get("database_doc_mode", "overview_plus_groups") == "overview_plus_groups"
        and (
    ...
    buckets: list[DocBucket] = [
        DocBucket(
            ...
            required_sections=[
                "overview", "storage_model", "schema_group_index",
                "high_level_er_diagram", "cross_group_relationships",
    ...
    for index, group in enumerate(getattr(db_scan, "groups", []) or [], start=1):
        ...
        buckets.append(
            DocBucket(
                ...
                required_sections=[
                    "overview", "models_tables", "relationships",
                    "indexes_constraints", "used_by", "group_diagram",
                ],
                required_diagrams=["er_diagram"],
                generation_hints={
                    "include_database_context": True,
    ...
    if len(db_scan.migration_files) >= 3 or len(getattr(db_scan, "knex_artifacts", [])) >= 4:
        ...
        buckets.append(
            DocBucket(
                ...
                + sorted({artifact.file_path for artifact in getattr(db_scan, "knex_artifacts", [])[:20]}),
                required_sections=[
                    "overview", "migration_strategy", "notable_migrations",
                    "query_patterns", "performance_notes",
                ],
                generation_hints={
                    "include_database_context": True,
                    "prompt_style": "database_group",
...
def _build_runtime_buckets(scan: RepoScan, cfg: dict[str, Any]) -> list[DocBucket]:
    ...
    if cfg.get("runtime_doc_mode", "dedicated_pages") != "dedicated_pages" or not runtime_scan:
        return []
    ...
    realtime_files = sorted({item.file_path for item in runtime_scan.realtime_consumers})
    ...
    buckets: list[DocBucket] = [
        DocBucket(
            ...
            required_sections=[
                "overview", "runtime_surfaces", "execution_map",
                "schedulers", "realtime_surfaces", "operational_notes",
            ],
    ...
    if task_files:
        buckets.append(
            DocBucket(
                ...
                required_sections=[
                    "overview", "tasks", "queues_retries", "producers_consumers", "schedule_sources",
                ],
                generation_hints={
    ...
    if scheduler_files:
        buckets.append(
            DocBucket(
                ...
                required_sections=[
                    "overview", "registered_schedules", "invoked_targets", "safety_notes",
                ],
                generation_hints={
                    "prompt_style": "runtime",
                    "icon": "clock",
    ...
    if realtime_files:
        buckets.append(
            DocBucket(
                ...
                required_sections=[
                    "overview", "consumers_routes", "group_flow", "auth_path", "redis_channel_layer",
                ],
                generation_hints={
                    "prompt_style": "runtime",
                    "icon": "clock",
...
def _build_graphql_buckets(scan: RepoScan, cfg: dict[str, Any]) -> list[DocBucket]:
    ...
    buckets: list[DocBucket] = [
        DocBucket(
            ...
            owned_files=files,
            required_sections=[
                "overview", "schema_roots", "queries_mutations", "resolver_flow", "related_models",
            ],
            generation_hints={
