...
- `deepdoc/planner/unit_boundaries.py` — Slice B deterministic semantic refinement, using only existing call-graph/topology/flow evidence, no LLM calls, no GitNexus. `refine_unit_ownership()` runs only when `build_planning_units()` produced 2+ named units; it conservatively moves an unclaimed `core` file into exactly one named unit when the file's combined call-graph/topology-cluster/flow-co-occurrence affinity score clears a fixed minimum and beats the runner-up by a fixed margin ratio — otherwise (including ties, weak signal, or a topology-foundational file) the file stays in `core`. A `file_services`-assigned file is never a candidate; that ownership is a hard anchor. `compute_boundary_stubs()` aggregates a bounded, serializable `BoundaryStub` (remote unit slug, direction, aggregate score/call count, evidence kinds, at most a few flow labels — never remote file paths, symbols, or raw edges) for a unit's *current* file membership against every other baseline unit; it is recomputed fresh on every call in `engine.py::_plan_unit_with_retry` (including every retry-split), so a split child never inherits a stale parent aggregate. `format_boundary_stubs()` renders the compact optional `cross_unit_context` section that `_plan_local`'s CLASSIFY prompt includes through the normal `fit_prompt_sections` optional-section path (can be omitted under a tight budget, never required).
