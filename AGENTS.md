...
- `deepdoc/generator/claims.py` — `ClaimExtractor`/`ClaimValidator`; claim invalidity is applied directly to the in-memory `ValidationResult` before ledger/summary status is computed. `inject_source_citations()` must skip YAML frontmatter, fenced code, and existing links; it emits commit-pinned GitHub links only for normalized known remotes and commits.
...
- `deepdoc/site/builder/next_builder.py` — **canonical site builder**: `build_next_from_plan()` copies the Next.js + Fumadocs shell template from `next_template/` into configured `site_dir` (`deepdoc-site/` by default), writes `deepdoc.config.json` and `.env.local` pointing at configured `output_dir`. `mkdocs_builder.py` was deleted in `b75b4eb`; this is the only site builder.
  ...
  - **Brand colours have a single owner**: `resolve_colors()` in `next_builder.py`. They are written only to `deepdoc.config.json` and injected into `<head>` by `app/layout.tsx` at request time — never baked into `globals.css`. `DEFAULT_CONFIG` ships `site.colors.*` as `""`, so the key exists and a plain `.get(key, default)` returns `""`; `resolve_colors` treats empty/invalid as unset (this previously emitted the invalid declaration `--brand: ;`).
  - **Fumadocs v15 CSS tokens are `--color-fd-*` and their values are complete colours.** Never write `--fd-<colour>` or wrap a token in `hsl(...)` — both are dead and get dropped by the browser. Size vars (`--fd-sidebar-width`, `--fd-toc-width`, `--fd-layout-offset`) are still valid and must be left alone. Fumadocs defines its tokens inside `@layer theme`/`@layer utilities`, so the template's unlayered `:root`/`.dark` blocks win without `!important`.
  - Everything under `site.*` (`theme`, `chrome`, `nav`, `labels`, brand assets) is **live-appliable**: it is resolved in `next_builder.py`, written to `deepdoc.config.json`, and read by the template per request. Never bake any of it into `globals.css` — that would require regeneration to change. Settings that alter *what is documented* (`exclude`, scan limits, model) are architectural and are deliberately not handled at serve time.
  - Colour presets are vendored as plain CSS in `deepdoc/site/builder/presets.py`. Seven ship; `solar` (a layer that `@import`s neutral, with `oklch()` colours) and `shadcn` (no dark block at all) are excluded because either would break dark mode. A test asserts every vendored preset has both modes.
  - Theme precedence is **preset → brand → explicit `site.theme.tokens`**, all emitted unlayered so they beat Fumadocs' own tokens without `!important`.
  - `site.repo_url` falls back to the git `origin` remote (`_detect_repo_url`), normalising scp-style URLs to https. `resolve_chrome` takes the *resolved* URL so an auto-detected repo still enables the edit link.
  - Nav overrides (`apply_nav_overrides`) run as four ordered passes — rename, hide, extra, pin/order — over the tree from `_build_nav`, all derived from the saved plan. A stale entry warns and is skipped; it must never raise.
  - Search is a **static** index: `app/api/search/route.ts` uses `staticGET` + `force-static` because the site builds with `output: 'export'`. Indexed prose is capped (`MAX_INDEXED_CHARS`) — uncapped, a 48-page repo shipped a ~3 MB download to every visitor.
  - **`deepdoc serve` and `deepdoc deploy` re-apply `.deepdoc.yaml` before running Next**, via `cli.py::_resync_site_from_config`. It loads the saved plan with `persistence_v2.load_plan` and calls `build_next_from_plan` — no planning, no scan, no LLM. This is what makes config-only changes visible without regenerating docs. It must run *before* `_ensure_node_modules`, since it rewrites `package.json`. A missing or pre-v2 plan warns and serves what is on disk rather than failing.
  - `lib/config.ts::getConfig()` must **not** cache at module scope. `deepdoc.config.json` is read via `fs`, not imported, so a module-level cache is never invalidated and would pin the config for the life of the `next dev` process.
  - The template deliberately has **no Tailwind** (`postcss.config.mjs` ships empty `plugins` to shadow ancestor configs — see `eef8106`). Do not `@import 'tailwindcss'` or a `fumadocs-ui/css/*.css` preset: presets are Tailwind `@theme` blocks and would half-apply, breaking light mode.
...
- Hierarchical `nav_structure` groups must retain their parent as a reachable page. The site builder emits recursive sections containing the parent page plus children; when a final child is removed, degrade the group to the surviving parent slug rather than dropping the parent from nav.
