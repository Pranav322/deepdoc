...
- `deepdoc/planner/partitioning.py` — `PlanningUnit` (carries `coarse: bool` and `boundary_stubs: tuple[Any, ...]` — see `unit_boundaries.py`), `build_planning_units()` (groups files by `scan.file_services`; unclaimed files form `core`; zero/one named service remains one compatibility unit), `make_sub_scan()` (isolates all file-scoped evidence, trims topology, drops the global call graph, and recomputes languages), `split_planning_unit()`/`next_split()` (deterministic splitting), `unit_likely_fits_budget()` (conservative preflight estimate), and `bound_planning_unit()` (pre-splits obviously oversized units; runtime prompt fitting remains authoritative). Config `planning_unit_max_files_seed` only shapes the first split guess.
- `deepdoc/planner/unit_boundaries.py` — Slice B deterministic semantic refinement, using only existing call-graph/topology/endpoint-bundle/runtime/flow evidence, no LLM calls, no GitNexus. `refine_unit_ownership()` runs only when `build_planning_units()` produced 2+ named units; it conservatively moves an unclaimed `core` file into exactly one named unit when the file's combined affinity score clears a fixed minimum and beats the runner-up by a fixed margin ratio. Affinity comes from five independent signals with fixed documented weights: raw local call edges, topology-cluster co-membership, flow co-occurrence, `EndpointBundle.evidence` file paths (only when the bundle's `handler_file` anchors to exactly one named unit and no evidence file belongs to another named unit), and `RuntimeTask.producer_files` (same single-anchor rule). One endpoint/runtime vote is sufficient on its own — flow candidates are optional and may be built after refinement — but the margin rule still keeps a helper claimed by two services in `core`. Schedulers, realtime consumers, and `scan.entry_points` deliberately contribute nothing: none carries a bounded dependent *file* list. `TopologyCluster.shared_dep_files` are foundational by construction, so they are already excluded — otherwise (including ties, weak signal, or a topology-foundational file) the file stays in `core`. A `file_services`-assigned file is never a candidate; that ownership is a hard anchor. `compute_boundary_stubs()` aggregates a bounded, serializable `BoundaryStub` (remote unit slug, direction, aggregate score/call count, evidence kinds, at most a few flow labels — never remote file paths, symbols, or raw edges). Flow labels are derived only from a flow's stable ID and must match a short lowercase alnum/dash identifier, degrading to a generic `flow` label otherwise: `FlowCandidate.title` is arbitrary upstream text (`endpoint_family` can be a route path) and must never reach a local prompt for a unit's *current* file membership against every other baseline unit; it is recomputed fresh on every call in `engine.py::_plan_unit_with_retry` (including every retry-split), so a split child never inherits a stale parent aggregate. `format_boundary_stubs()` renders the compact optional `cross_unit_context` section that `_plan_local`'s CLASSIFY prompt includes through the normal `fit_prompt_sections` optional-section path (can be omitted under a tight budget, never required).
