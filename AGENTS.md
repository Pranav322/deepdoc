...
### Planner
- `deepdoc/planner/engine.py` — repo-scan entrypoint and bucket-planning orchestration. `plan_docs()` is a thin orchestrator: it runs `run_phase2_scans()` exactly once on the full scan, calls `build_planning_units()`, then runs every unit through `bound_planning_unit()` (exact-token gate, not raw unit count) before deciding the path. Zero/one meaningful unit **and** it fits the model budget → plans the whole scan directly via `_plan_local(apply_global_stage=True)` (identical to pre-Slice-A behavior, no prefixing). Otherwise each unit is planned via `_plan_local(apply_global_stage=False)` against a `make_sub_scan()`-filtered view (a coarse unit — see below — skips straight to `_coarse_unit_plan()`, no LLM call), the results are merged with `merge_unit_plans()`, and `_apply_global_plan_stage()` (the global-only bucket injectors plus final nav/orphan/coverage shaping) runs exactly once on the merged plan against the full global scan, followed by one `validate_plan_contract()` call. `_plan_local()`'s `apply_global_stage` flag controls whether it runs that same global stage itself (single-unit path) or stops after the per-unit-safe local refinement steps — never rerun Phase 2 scans, callers own that.
- `deepdoc/planner/partitioning.py` — `PlanningUnit` (carries `coarse: bool`), `build_planning_units()` (groups `scan.file_summaries` by `scan.file_services`; unclaimed files go to a deterministic `core` unit; collapses to one unit when there's zero or one meaningful service — but callers must still run it through `bound_planning_unit()`, since a single meaningful service can itself be too large), `make_sub_scan()` (projects a `RepoScan` to one unit's files without mutating the source scan: filtered `TopologyMap` with per-cluster file lists trimmed — not just whole clusters kept/dropped — global `call_graph` dropped rather than filtered, `languages` recomputed from the unit's own files), `split_planning_unit()` (deepest-shared-directory grouping, falling back to stable numbered chunks for a flat unit that can't be split by path — a cheap seed, not proof of fit), `unit_fits_model_budget()` (the real acceptance gate: renders the actual required PROPOSE/ASSIGN sections and counts them with the real tokenizer), `bound_planning_unit()` (recursively splits via `split_planning_unit` until `unit_fits_model_budget` passes for every part; an indivisible file that still doesn't fit comes back `coarse=True`). Config `planning_unit_max_files_seed` (default 0/unset) only shapes the first split guess, never substitutes for the exact-token check.
- `deepdoc/planner/merge.py` — `merge_unit_plans()` deterministically merges per-unit `DocPlan`s: clones every bucket before rewriting (the caller's source plans are never mutated), namespaces every non-introduction bucket under `{unit-slug}/`, rewrites `depends_on`/`parent_slug`/nav references through the same slug map, and — as a defensive safety net for a real LLM propose step that volunteers its own introduction bucket per unit — keeps the first one found as the single global introduction and demotes every other one to a namespaced overview bucket. Does **not** validate the result itself (an intermediate merged plan has no introduction until `_apply_global_plan_stage` runs) — `plan_docs()` owns calling `_apply_global_plan_stage()` then `validate_plan_contract()` once. `missing_files()` reports any scan file owned by neither a bucket nor the skip/orphan lists; `plan_docs()` tops up any leftover into `orphaned_files` as an explicit, deterministic safety net.
