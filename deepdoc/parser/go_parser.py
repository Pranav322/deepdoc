...
def go_runtime_facts(content: str) -> tuple[GoRuntimeFact, ...]:
    ...
    program_scope = (0, root.end_byte)
    bindings: dict[tuple[tuple[int, int], str], list[tuple[int, str]]] = {}
    ...
    def scope_for(node) -> tuple[int, int]:
        current = node
    ...
    def add_binding(
        scope: tuple[int, int], name: str, position: int, role: str
    ) -> None:
        if name and name != "_":
            bindings.setdefault((scope, name), []).append((position, role))

    def binding_role(scope: tuple[int, int], name: str, position: int) -> str:
        events = bindings.get((scope, name), ())
        index = bisect_right(events, (position, "\uffff")) - 1
        if index >= 0:
            return events[index][1]
        if scope != program_scope:
            return binding_role(program_scope, name, position)
        return ""
    ...
    def constructor_role(value, scope: tuple[int, int], position: int) -> str:
        ...
        module_role = binding_role(scope, target_name(operand), position)
    ...
    def value_role(value, scope: tuple[int, int], position: int) -> str:
        name = target_name(value)
        if name:
            return binding_role(scope, name, position)
        return constructor_role(value, scope, position)
    ...
    for node in writes:
        ...
        for name, value in write_pairs(node):
            add_binding(
                ...
                name,
                node.start_byte,
                constructor_role(value, scope, node.start_byte),
            )
    ...
    def visit(node) -> None:
        if node.type == "go_statement":
            ...
            if (
                method == "AddFunc"
                and value_role(receiver, scope, node.start_byte) == "receiver:robfig_cron"
                and len(arguments) >= 2
            ):
                ...
                if (
                    every == "Every"
                    and value_role(every_receiver, scope, node.start_byte) == "receiver:gocron"
                    and every_args
                    and target
                ):
...
def _extract_type_spec(
    spec_node, parent_node, lines: list[str], symbols: list[Symbol]
) -> None:
        ...
        if child.type == "field_declaration_list":
            for field_node in child.children:
                if field_node.type == "field_declaration":
                    field_line = (
                        lines[field_node.start_point[0]].strip()
                        if field_node.start_point[0] < len(lines)
                        else ""
                    )
                    if field_line:
...
def _extract_interface_methods(iface_node, lines: list[str]) -> list[str]:
    ...
    for child in iface_node.children:
        if child.type in ("method_spec", "method_elem"):
            method_line = (
                lines[child.start_point[0]].strip()
                if child.start_point[0] < len(lines)
                else ""
            )
            if method_line:
...
def _extract_const_block(
    node, lines: list[str], symbols: list[Symbol], is_const: bool = True
) -> None:
        ...
        symbols.append(
            Symbol(
                ...
                body_preview=_body_preview(
                    node, lines, max_lines=min(len(specs) + 2, 15)
                ),
                start_line=node.start_point[0] + 1,
                end_line=node.end_point[0] + 1,
                fields=specs,
                ...
                visibility="exported"
                if (specs[0] and specs[0][0].isupper())
                else "unexported",
            )
        )
    else:
        ...
        if stripped.startswith("const (") or stripped == "const (":
            ...
            if const_names:
                has_iota = (
                    "iota"
                    in content[
                        content.find("const (") : content.find(
                            ")", content.find("const (")
                        )
                        + 1
                    ]
                ...
                if has_iota and len(const_names) > 1:
                    symbols.append(
                        Symbol(
                            ...
                            docstring=pending_doc,
                            body_preview="\n".join(
                                lines[i : i + min(len(const_names) + 2, 15)]
                            ),
                            start_line=i + 1,
                            fields=const_names,
                            is_exported=const_names[0][0].isupper()
                            if const_names[0]
                            else False,
                        )
                    )
